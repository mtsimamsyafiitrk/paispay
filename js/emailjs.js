...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (isLoggedIn()) {
    ...
    if (g.nama && g.taId) {
      document.getElementById('loginScreen').classList.add('hidden');
      // Ambil ulang semua data dari Supabase (sama seperti doLoginGuest)
      Promise.all([
        sb('students?select=*&nama=eq.' + encodeURIComponent(g.nama) + '&order=ta_id'),
        sb('transactions?select=*&nama=eq.' + encodeURIComponent(g.nama) + '&order=created_at.desc'),
        sb('academic_years?select=*'),
      ]).then(([allSiswa, txns, allTAs]) => {
        if (!allSiswa.length) throw new Error('Santri tidak ditemukan');
        const taMap = {};
        allTAs.forEach(t => { taMap[t.id] = t.label; });
        const siswa = allSiswa.find(s => s.ta_id === g.taId) || allSiswa[0];
        ...
        guestData = { siswa, taId: g.taId, taLabel: g.taLabel, txns, perTA, taMap };
