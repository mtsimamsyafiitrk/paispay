...
function showPage(id) {
  ...
  if(id==='tahun-ajaran') { renderTAList(); }
...
function isSameStudent(a, b) {
  // Jika dalam TA yang sama tapi kelas berbeda → pasti orang berbeda
  if (a.ta_id && b.ta_id && a.ta_id === b.ta_id && a.kelas !== b.kelas) return false;
...
// Load semua siswa dari semua TA → bangun studentIndex
let allStudentsAllTA = []; // cache flat list semua siswa semua TA

async function loadAllStudentsIndex() {
  try {
    const rows = await sb('students?select=*&order=nama');
    allStudentsAllTA = rows.map(r => ({
      nama: r.nama, kelas: r.kelas, nisn: r.nisn || '',
      spp: Number(r.spp)||0, pangkal: Number(r.pangkal)||0,
    ...
    buildStudentIndex();
  } catch(e) { console.warn('loadAllStudentsIndex error:', e.message); }
}

function buildStudentIndex() {
  studentIndex = {};
  // Kelompokkan: coba NISN dulu, lalu fuzzy nama
  allStudentsAllTA.forEach(s => {
    // Cari grup yang cocok
    let found = null;
    for (const key of Object.keys(studentIndex)) {
      const grp = studentIndex[key];
      if (grp.records.some(r => isSameStudent(r, s))) {
        found = key;
        break;
      }
    }
    if (found) {
      studentIndex[found].records.push(s);
      // Update NISN ke group jika belum ada
      if (s.nisn && !studentIndex[found].nisn) studentIndex[found].nisn = s.nisn;
    } else {
      const key = canonicalKey(s);
      studentIndex[key] = {
        nisn: s.nisn || '',
        nama_canonical: normNama(s.nama),
        nama_display: s.nama,
        records: [s],
      };
    }
...
// Dapatkan semua record lintas TA untuk satu siswa (dari appState)
function getStudentAllTA(namaOrNisn) {
  const cur = appState.students.find(s =>
    s.nama === namaOrNisn || (namaOrNisn && s.nisn && s.nisn === namaOrNisn)
  );
  if (!cur) return { records: [], totalTunggakanCross: 0 };

  // Cari di index semua TA
  for (const key of Object.keys(studentIndex)) {
    const grp = studentIndex[key];
    if (grp.records.some(r => isSameStudent(r, cur))) {
      // Hitung total tunggakan lintas TA (kecuali TA aktif)
      const crossRecords = grp.records.filter(r => r.ta_id !== activeTA?.id);
      const totalTunggakanCross = crossRecords.reduce((sum, r) => {
        const sppT = MONTHS.filter(m => !r.spp_paid_months.includes(m)).length * (r.spp||0);
        const pangkalT = Math.max(0, (r.pangkal||0) - (r.pangkal_paid||0));
        return sum + sppT + pangkalT;
      }, 0);
      return { ...grp, totalTunggakanCross };
    }
  }
  return { records: [cur], totalTunggakanCross: 0 };
}

// Cari semua transaksi siswa lintas TA dari Supabase
async function getAllTransactionsByStudent(s) {
  const queries = [];
  // Query by NISN jika ada
  if (s.nisn) {
    queries.push(sb('transactions?select=*&nama=eq.' + encodeURIComponent(s.nama) + '&order=created_at.desc'));
  }
  // Tambah query nama-nama mirip dari index
  const grp = getStudentAllTA(s.nama);
  const namaSet = new Set(grp.records.map(r => r.nama));
  const allTxns = (await Promise.all(
    [...namaSet].map(nama =>
      sb('transactions?select=*&nama=eq.' + encodeURIComponent(nama) + '&order=created_at.desc').catch(()=>[])
    )
  )).flat();
  // Deduplikasi
  const seen = new Set();
  return allTxns.filter(t => {
    const key = t.ta_id + '_' + t.time + '_' + t.nominal;
...
function pangkalTunggakan(s) {
  // Pangkal hanya 1x selama studi
  // pangkal_paid di tiap TA adalah nilai KUMULATIF (bukan tambahan per TA)
  // Gunakan MAX dari semua TA untuk menghindari double count data lama
  if (!s.pangkal || s.pangkal === 0) return 0;

  // Cari semua record siswa ini lintas TA
  const allRecords = allStudentsAllTA.length
    ? allStudentsAllTA.filter(r => isSameStudent(r, s))
    : [s];

  // Ambil nilai pangkal_paid TERTINGGI dari semua TA (karena bersifat kumulatif)
  const totalPaid = allRecords.reduce((max, r) => Math.max(max, r.pangkal_paid||0), 0);

  // Nominal pangkal diambil dari record yang punya nilai terbesar
  const nominalPangkal = allRecords.reduce((max, r) => Math.max(max, r.pangkal||0), s.pangkal);

  return Math.max(0, nominalPangkal - totalPaid);
}
function crossTATunggakan(s) {
  // Hanya hitung SPP lintas TA — pangkal sudah dihitung kumulatif di pangkalTunggakan()
  return (s.cross_ta_debt || []).reduce((sum, d) => {
    const sppDebt = (d.spp_months?.length || 0) * (d.spp || 0);
    return sum + sppDebt;
  }, 0);
}
function totalTunggakan(s) { return sppTunggakan(s) + pangkalTunggakan(s) + crossTATunggakan(s); }
