...
function renderPaymentItems(student) {
  ...
  let html = activeItems.map(item => {
    ...
    if (item.id === 'pangkal' && student && student.pangkal > 0) {
      ...
      // Ambil MAX pangkal_paid dari semua TA (kumulatif, bukan tambahan)
      const allRec = allStudentsAllTA.length
        ? allStudentsAllTA.filter(r => isSameStudent(r, student))
        : [student];
      const totalPaid = allRec.reduce((max, r) => Math.max(max, r.pangkal_paid||0), 0);
      const nominalPangkal = allRec.reduce((max, r) => Math.max(max, r.pangkal||0), student.pangkal);
      if (sisa <= 0) {
  ...
  // ── Tunggakan lintas TA ──
  // Gabungkan cross_ta_debt dari appState + data real dari allStudentsAllTA
  const crossDebtFromIndex = [];
  if (student && allStudentsAllTA.length) {
    const otherRecords = allStudentsAllTA.filter(r =>
      r.ta_id !== activeTA?.id && isSameStudent(r, student)
    );
    otherRecords.forEach(r => {
      const taLbl = allTA.find(t => t.id === r.ta_id)?.label || r.ta_id;
      const unpaidSPP = MONTHS.filter(m => !r.spp_paid_months.includes(m));
      // Pangkal tidak dimasukkan — sudah dihitung kumulatif di pangkalTunggakan()
      if (unpaidSPP.length > 0 && r.spp > 0) {
        crossDebtFromIndex.push({
          ta_id: r.ta_id, ta_label: taLbl,
          spp_months: unpaidSPP, spp: r.spp,
          pangkal_sisa: 0,
        });
      }
    });
  }
  // Merge: prioritaskan data dari index (lebih akurat), fallback ke cross_ta_debt
  const crossDebt = crossDebtFromIndex.length > 0
    ? crossDebtFromIndex
    : (student?.cross_ta_debt || []);
  const crossItems = [];
  crossDebt.forEach(d => {
    const taLbl = d.ta_label || 'TA Lalu';
    // Hanya SPP per bulan — pangkal sudah terakumulasi di item pangkal TA aktif
    (d.spp_months || []).forEach(m => {
      crossItems.push({
        crossId: `cross_spp_${d.ta_id}_${m}`,
        label: `SPP ${MONTH_FULL[m]||m} (TA ${taLbl})`,
        nominal: d.spp || 0,
        ta_id: d.ta_id, ta_label: taLbl,
        type: 'cross_spp', bulan: m,
      });
    });
  });

  if (crossItems.length) {
    html += `<div style="margin-top:14px;padding-top:12px;border-top:2px dashed var(--danger);border-top-style:dashed;">
      <div style="font-size:11px;font-weight:700;color:var(--danger);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;">⚠️ Tunggakan Tahun Ajaran Sebelumnya</div>
      ${crossItems.map(ci => `
      <div class="pay-item" style="border-left:3px solid var(--danger);padding-left:10px;margin-bottom:6px;">
        <input type="checkbox" id="chk_${ci.crossId}" onchange="calcTotal()">
        <div class="pay-item-info">
          <div class="pay-item-name" style="font-size:13px;">${ci.label}</div>
          <div class="pay-item-amount" style="color:var(--danger);">${rp(ci.nominal)}</div>
        </div>
      </div>`).join('')}
    </div>`;
    // Simpan crossItems ke variabel global agar bisa dibaca submitPayment
    window._crossItemsRender = crossItems;
  } else {
    window._crossItemsRender = [];
  }
...
function calcTotal() {
  ...
  // Hitung cross TA items
  (window._crossItemsRender || []).forEach(ci => {
    const chk = document.getElementById('chk_' + ci.crossId);
    if (chk?.checked) total += ci.nominal;
  });
...
async function submitPayment() {
  ...
  // Kumpulkan cross TA items yang dicentang
  const crossItemsPaid = (window._crossItemsRender || []).filter(ci =>
    document.getElementById('chk_' + ci.crossId)?.checked
  );
  crossItemsPaid.forEach(ci => {
    items.push({ id: ci.crossId, name: ci.label, amount: ci.nominal, crossItem: ci });
  });
  ...
  items.forEach(it => {
    ...
    if (it.id === 'pangkal') {
      ...
      // Sync ke allStudentsAllTA agar pangkalTunggakan() langsung akurat
      const idxAll = allStudentsAllTA.findIndex(r =>
        r.ta_id === activeTA?.id && isSameStudent(r, appState.students[si])
      );
      if (idxAll >= 0) allStudentsAllTA[idxAll].pangkal_paid = appState.students[si].pangkal_paid;
    }
    // Kurangi/hapus dari cross_ta_debt (hanya untuk SPP)
    if (it.crossItem && it.crossItem.type === 'cross_spp') {
      const ci = it.crossItem;
      const debt = appState.students[si].cross_ta_debt || [];
      const dIdx = debt.findIndex(d => d.ta_id === ci.ta_id);
      if (dIdx >= 0) {
        debt[dIdx].spp_months = (debt[dIdx].spp_months || []).filter(m => m !== ci.bulan);
        // Hapus entry jika sudah tidak ada tunggakan SPP
        if (!debt[dIdx].spp_months?.length) {
          debt.splice(dIdx, 1);
        }
        appState.students[si].cross_ta_debt = debt;
      }
    }
  });
  ...
  const kwtData = {
    ...
    ta_id: activeTA?.id || null, ta_label: activeTA?.label || '',
