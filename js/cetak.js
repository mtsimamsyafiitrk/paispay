...
async function exportSiswaExcel(mode) {
  xlsxReady(async () => {
    ...
    try {
      let rows = [];
      if (mode === 'aktif') {
        const taLbl = activeTA?.label || '—';
        rows = appState.students.map(s => siswaToRow(s, taLbl));
      } else {
        const all = await sb('students?select=*&order=nama');
        const taMap_ = {};
        allTA.forEach(t => { taMap_[t.id] = t.label; });
        rows = all.map(s => siswaToRow({
          ...s,
        ...
        }, taMap_[s.ta_id] || s.ta_id));
      }
      const ws = XLSX.utils.json_to_sheet(rows);
      // Lebar kolom otomatis
      ws['!cols'] = [8,24,6,12,10,12,12,12,8,32,12,10].map(w => ({wch: w}));
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Data Santri');
      const tgl = new Date().toISOString().slice(0,10);
      downloadXlsx(wb, `SiPay_DataSantri_${mode}_${tgl}.xlsx`);
...
async function exportTransaksiExcel(mode) {
  xlsxReady(async () => {
    toast('⏳ Menyiapkan data...');
    try {
      let url = 'transactions?select=*&order=created_at.desc';
      if (mode === 'aktif') url += '&ta_id=eq.' + activeTA?.id;
      const rows = await sb(url);
      const taMap_ = {};
      allTA.forEach(t => { taMap_[t.id] = t.label; });
      const fmt = n => Number(n||0).toLocaleString('id-ID');
      const data = rows.map(r => ({
        'Tahun Ajaran': taMap_[r.ta_id] || '—',
      ...
      ws['!cols'] = [10,12,6,24,6,32,12,24].map(w => ({wch: w}));
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Transaksi');
      downloadXlsx(wb, `SiPay_Transaksi_${mode}_${new Date().toISOString().slice(0,10)}.xlsx`);
      toast('✅ Export berhasil!');
    } catch(e) { toast('⚠️ Gagal export: ' + e.message); }
  });
}
...
async function exportKuitansiExcel(mode) {
  xlsxReady(async () => {
    ...
    try {
      ...
      const ws = XLSX.utils.json_to_sheet(data);
      ws['!cols'] = [16,12,10,24,6,12,36,12,24,8,8,16].map(w => ({wch: w}));
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Kuitansi');
      downloadXlsx(wb, `SiPay_Kuitansi_${mode}_${new Date().toISOString().slice(0,10)}.xlsx`);
      toast('✅ Export berhasil!');
    } catch(e) { toast('⚠️ Gagal export: ' + e.message); }
  });
}
...
async function exportBackupLengkap() {
  xlsxReady(async () => {
    ...
    try {
      ...
      wsSiswa['!cols'] = [10,24,6,12,10,12,12,12,8,32,12,10].map(w => ({wch: w}));
      ...
      wsTxn['!cols'] = [10,12,6,24,6,32,12,24].map(w => ({wch: w}));
...
function buildSuratHTML(s, tgl) {
  ...
  const P = getProfil();
  const taLabel = activeTA?.label || '2025/2026';
  let no = 0;
  return `
  <div class="pdf-preview" id="suratPreview">
    ...
    <div class="surat-body">
      ...
      <p style="text-align:justify;line-height:1.8;margin-bottom:12px;font-size:11pt;">
        Dengan hormat, bersama surat ini kami sampaikan informasi pembayaran santri yang bersangkutan untuk Tahun Ajaran <strong>${taLabel}</strong> sebagaimana tercantum di bawah ini:
      </p>
