...
async function cetakKuitansiById(nama, timeStr) {
  ...
  try {
    ...
    const match = rows.find(r => r.ta_label === (activeTA?.label||'')) || rows[0];
    if (match) {
      // Tandai dicetak
...
async function konfirmasiHapusKwt() {
  ...
  try {
    ...
    if (kwt.nama && kwt.ta_id && kwt.items?.length) {
      const siswaRows = await sb('students?select=*&nama=eq.' + encodeURIComponent(kwt.nama) + '&ta_id=eq.' + kwt.ta_id);
      if (siswaRows.length) {
        const s = siswaRows[0];
        ...
        if (changed) {
          await sb(
            'students?nama=eq.' + encodeURIComponent(kwt.nama) + '&ta_id=eq.' + kwt.ta_id,
            'PATCH',
            { spp_paid_months: sppMonths, pangkal_paid: pangkalPaid },
            { 'Prefer': 'return=minimal' }
          );
          const si = appState.students.findIndex(s => s.nama === kwt.nama && kwt.ta_id === activeTA?.id);
          if (si >= 0) {
            appState.students[si].spp_paid_months = sppMonths;
            appState.students[si].pangkal_paid    = pangkalPaid;
          }
          const ai = allStudentsAllTA.findIndex(r => r.nama === kwt.nama && r.ta_id === kwt.ta_id);
          if (ai >= 0) {
...
async function loadRiwayatKuitansi() {
  ...
  const taId    = document.getElementById('kwt_ta')?.value || '';
  ...
  try {
    ...
    const taUnik = new Set(rows.map(r => r.ta_id)).size;
