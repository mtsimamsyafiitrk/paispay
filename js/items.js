...
function showDetail(nama) {
  ...
  // Data lintas TA dari index
  const grp = getStudentAllTA(s.nama);
  const otherTA = (grp.records || []).filter(r => r.ta_id !== activeTA?.id);
  const taMap_ = {};
  ...
  // Riwayat TA lain
  const otherTAHtml = otherTA.length ? `
    <div style="margin-top:14px;">
      <div style="font-weight:700;font-size:12px;color:var(--primary);margin-bottom:8px;">📂 Riwayat Tahun Ajaran Lain</div>
      ${otherTA.map(r => {
        const tl = taMap_[r.ta_id] || r.ta_id;
        const sT = MONTHS.filter(m => !r.spp_paid_months.includes(m)).length * (r.spp||0);
        const pT = Math.max(0, (r.pangkal||0) - (r.pangkal_paid||0));
        return `<div style="background:var(--bg);border-radius:8px;padding:10px 12px;margin-bottom:6px;font-size:12px;">
          <div style="font-weight:600;color:var(--primary);">TA ${tl} — Kelas ${r.kelas}</div>
          <div style="margin-top:4px;color:${sT+pT>0?'var(--danger)':'var(--primary-light)'};">
            ${sT+pT > 0 ? `⚠️ Tunggakan: ${rp(sT+pT)}` : '✅ Lunas'}
          </div>
        </div>`;
      }).join('')}
    </div>` : '';
