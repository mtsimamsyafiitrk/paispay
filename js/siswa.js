...
async function renderLogSiswa(resetPage = true) {
  ...
  const selTA = document.getElementById('logTAFilter');
  if (selTA && selTA.options.length <= 1 && allTA.length) {
    allTA.forEach(t => {
      const opt = document.createElement('option');
      opt.value = t.id;
      opt.textContent = 'TA ' + t.label;
      selTA.appendChild(opt);
    });
  }
  const taId = selTA?.value || '';
  ...
  try {
    ...
    const taMap_ = {};
    allTA.forEach(t => { taMap_[t.id] = t.label; });
        ...
        ${txns.map(r => {
          ...
          return `<div style="display:flex;align-items:center;gap:12px;padding:10px 12px;border-radius:10px;background:#fafafa;border:1px solid var(--border);margin-bottom:6px;">
            ...
            <div style="text-align:right;white-space:nowrap;">
              ...
              <div style="font-size:10px;color:#aaa;margin-top:2px;">TA ${taLbl}</div>
            </div>
          </div>`;
        }).join('')}
      </div>`).join('');
...
function renderTunggakanDetail(s) {
  ...
  // Section cross TA debt
  const crossDebt = s.cross_ta_debt || [];
  const crossSection = crossDebt.length ? `
    <div style="margin-bottom:8px;font-weight:700;font-size:13px;color:var(--danger);">📂 Tunggakan Tahun Ajaran Sebelumnya</div>
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:20px;">
      ${crossDebt.map(d => {
        const sppDebt     = (d.spp_months?.length || 0) * (d.spp || 0);
        const pangkalDebt = d.pangkal_sisa || 0;
        const totalDebt   = sppDebt + pangkalDebt;
        if (totalDebt === 0) return '';
        return `<div style="background:var(--danger-pale);border:1px solid #fca5a5;border-radius:12px;padding:14px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <div style="font-weight:700;font-size:13px;color:var(--danger);">📅 TA ${d.ta_label || 'TA Lalu'}</div>
            <div style="font-weight:800;font-size:14px;color:var(--danger);">${rp(totalDebt)}</div>
          </div>
          ${pangkalDebt > 0 ? `
          <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text);margin-bottom:4px;">
            <span>🏫 Pangkal belum lunas</span><strong>${rp(pangkalDebt)}</strong>
          </div>` : ''}
          ${sppDebt > 0 ? `
          <div style="font-size:12px;color:var(--text);margin-bottom:4px;">
            📅 SPP belum bayar (${d.spp_months.length} bulan × ${rp(d.spp || 0)}):
          </div>
          <div style="display:flex;flex-wrap:wrap;gap:4px;">
            ${(d.spp_months || []).map(m => `<span style="background:#fff;border:1px solid #fca5a5;color:var(--danger);border-radius:6px;padding:2px 8px;font-size:11px;font-weight:600;">${m}</span>`).join('')}
          </div>` : ''}
        </div>`;
      }).join('')}
    </div>` : '';
